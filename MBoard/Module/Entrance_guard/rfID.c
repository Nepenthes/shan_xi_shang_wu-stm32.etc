...
osThreadId tid_rfID_Thread;
osThreadDef(rfID_Thread,osPriorityNormal,1,1536);
...
//	char disp[dpSize];
	...
	static uint8_t KD_cnt;
	static bool KD_FLG = true;
	...
	for(;;){
		...
		if(KD_FLG){
			...
			if(KD_cnt)KD_cnt --;
				...
				memset(ucArray_ID,0,4 * sizeof(u8));
			}
		}
...
//				memset(disp,0,dpSize * sizeof(char));			/**指纹模块占用串口，仅调试使用**/
...
void rfIDThread_Active(void){
	...
	rfID_Init();
	tid_rfID_Thread = osThreadCreate(osThread(rfID_Thread),NULL);
}
