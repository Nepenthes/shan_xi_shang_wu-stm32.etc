...
osMessageQDef(MsgBox_MTFID, 1, &FID_MEAS);             // 消息队列定义
osMessageQId  MsgBox_DPFID;		
osMessageQDef(MsgBox_DPFID, 1, &FID_MEAS);             // 消息队列定义
...
void fingerID_Active(void){
	...
	FID_pool = osPoolCreate(osPool(FID_pool));	//创建内存池
	MsgBox_FID = osMessageCreate(osMessageQ(MsgBox_FID), NULL);	//创建消息队列
	MsgBox_MTFID = osMessageCreate(osMessageQ(MsgBox_MTFID), NULL);	//创建消息队列
	MsgBox_DPFID = osMessageCreate(osMessageQ(MsgBox_DPFID), NULL);	//创建消息队列
